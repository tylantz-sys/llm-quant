...
| true | paper | Alpaca 5‑min bars (`market_data_intraday`) + daily macro | Paper portfolio updates only | **Synthetic partial TP + trailing** (no native OCO orders) | Yes | Yes (per 5‑min slot) | Intraday context logged; no Alpaca order state |
